...
\addcontentsline{toc}{part}{ASSIGNMENT 1}
\part*{Assignment 1}
...
Part A of the first assignment consists of performing a modal analysis on a cantilever beam, by making reference to the standing wave solution of a slender beam in bending vibration.\\
In particular, the objective of the first part is to derive the natural frequencies and the mode shapes of the structure.\\ 
...
The second section of the part A consists in computing the FRFs for four different pairs of input and output positions.\\
...
Part B of the first assignment consists in a modal parameters identification starting from real experimental data coming from vibration measurements of a rail wheel.
\begin{figure}[H]
    \centering
...
Similarly to Part A, numerical FRFs have to be computed starting from experimental data, exploitng an error minimisation procedure.
\subsection{Initial guesses estimation}
\phantomsection
Starting from data shown above, initial guesses are computed exactly as in Part A, exception made for the calculation of natural frequencies.\\
...
\subsection{Mode shapes reconstruction}
\phantomsection
Once we have obtained the numerical FRFs, we can reconstruct the mode shapes using the following formula (exactly as in the Part A \hyperref[Mode shapes reconstruction]{Section 3.3}):
\begin{equation*}
...
\thispagestyle{empty}
\addcontentsline{toc}{part}{ASSIGNMENT 2}
\part*{Assignment 2}
